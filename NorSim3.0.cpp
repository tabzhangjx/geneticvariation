...
//单点变异/短插入删除/长插入删除/继承；（AB/BB）（A不变/B变）（原先短变异十位以内）（密集区（指定区域进行密集变异，且变异过的不能再变异））
//倒置/微卫星
//注意到可拓展性（可以添加新的变异类型）
...
#define INIT_SEQ(seq) (seq).s = 0; (seq).l = (seq).max = 0//关于序列结构的初始化，初始序列为0，长度等于大小等于0；//用于存储.fa中信息；
...
static int SEQ_BLOCK_SIZE = 512;			//用于扩容的一个序列块；
...
static double zt_stdfun(){
	...
	return s;
}//返回一个-2与2之间的双精度数，公式的作用是？

/**********************************************/
FILE *FileOpen(char *fileName, const char *mode)//参数为待打开文件文件名及读写控制参数；
{
	FILE *fp;
	fp = fopen (fileName, mode);//以mode形式打开文件
	if (fp == NULL)
	{
...
/**********************************************/
int gch_to_num(char orgch){//将碱基信息转化为数字；
	int ch;
...
///**************************************************************
int seq_read_fasta(FILE *fp, seq_t *seq, char *chrname, char *comment)//ingenfp：读取源；//&seq： //ref_name：//0
{
	int c, l, max, n;
	char *p, cht[2];
	
	c = 0;
	while (!feof(fp) && fgetc(fp) != '>');//判断是否到达文件尾并读字符；
	if (feof(fp)) return -1;
	p = chrname;
	while (!feof(fp) && (c = fgetc(fp)) != ' ' && c != '\t' && c != '\n')
		if (c != '\r') *p++ = c;
	*p = '\0';															//将.fa文件的基因源的染色体号给入*p，即ref_name;
	//cout<<endl<<"ref_name="<<chrname<<endl;
	if (comment) {														//comment初值为零且没有进行改变，所以似乎没起到什么作用；
		p = comment;
		if (c != '\n') {
	...
	n=0; cht[1] = '\0';
	l = 0; max = seq->max;
	while (!feof(fp) && (c = fgetc(fp)) ) {								//未到达文件尾并将下一个字符读入c；
		if (isalpha(c) || c == '-' || c == '.') {
			if (l + 1 >= max) {
				max += SEQ_BLOCK_SIZE;
				seq->s = (char *)realloc(seq->s, sizeof(char) * max);	//对seq.s新分配内存；
			}
			cht[0] = seq->s[l++] = _toupper((char)c);					//写入seq.s;
			if (strstr("ACGT", cht)) n++;								//计数；
		}
	}
	seq->s[l] = 0;
	seq->max = max; 
...
/*******************************************************/
void init_indel_sim( seq_t *seq, mutseq_t *hap1, mutseq_t *hap2, indel_t *indelp)//&seq, &sim[0], &sim[1], &indelp  //seq：传入源基因信息；//
{
	...
	ret[0] = hap1; ret[1] = hap2;//sim[0],sim[1];
	ret[0]->l = seq->l; ret[1]->l = seq->l;
	ret[0]->max = seq->max; 
	ret[1]->max = seq->max;
	ret[0]->s = (mut_t *)calloc(seq->max, sizeof(mut_t));//指明用于存储每位信息的三十二位数所占的总大小；
	ret[1]->s = (mut_t *)calloc(seq->max, sizeof(mut_t));//双链；

	n=0;
	for (i = 0; i < seq->l; ++i) {
		ret[0]->s[i] = ret[1]->s[i] = (mut_t)gch_to_num(seq->s[i]);//一个函数调用，将碱基信息转化为数字；
		if ( (ret[0]->s[i]&0xf) < 4) n++;	//？
	}
	ret[0]->gnum = n;  ret[1]->gnum = n;//未变异数目初始化为n；
	ret[0]->vnum = 0;  ret[1]->vnum = 0;//已变异数目初始化为0；
	
	// handle for long delete area from long indel file
	if (!indelfp || !LONG_INDEL) return;//如果没有插入缺失设置与长插入缺失设置文件，则结束函数调用；
	idstr = new char [100000];
	idmark = new char [100000];
	n1=0; n2 = 0; del_no = 0; in_no = 1; addnum=1;
	while ( fscanf(indelfp, "%d %d %d %d %d %d %s\n", &idno, &idbeg, &idlen, &gtype, &idnum, &idvar, idstr) != EOF ){//类似正则表达式，读取设置文件的信息并赋给变量；
		if (idlen<0) {  // long delete from long indel file!!!
			...
			case 0: break;
			case 1: h = 0;
					for (int i=0; i<idlen; i++){//idlen：单次读入总字符数；
						indelmk = ret[h]->s[idbeg+i-1]& LONGINSERT;
						if (indelmk == LONGINSERT) {
							ret[h]->s[idbeg + i - 1] = (ret[h]->s[idbeg + i - 1] & NOTDELETE) | GTYPEAB;
						}
...
{	char c, cc, fhead[50];
	...
	while ((c = getopt(argc, argv, "r:R:X:D:B:I:A:o:")) >= 0) {
		...
		case 'o': strcpy(outresultf, optarg);break;//输出文件的文件名；
		default : break; 
		}
		if (cc) {optind = argc; break;}
	}
	if (optind != (argc-2) ) return simu_usage();
	strcpy(ingenf, argv[optind]);//ingenf中存放.fa文件名；
	strcpy(outsimf, argv[optind+1]);//outsimf中存放.sim文件名；
	for (i = 0; i < strlen(outsimf); i++) {
		if (outsimf[i] == '.') break;
	}
	if (i < strlen(outsimf)) {
		strncpy(fhead, &outsimf[0], i); fhead[i] = '\0';
	}//将.sim文件的文件名拷贝给fhead；
	else strcpy(fhead, outsimf); //此时说明没有后缀名，直接拷贝即可；
	if (strlen(outresultf)==0) sprintf(outresultf, "%s_simout.txt", fhead); //若不指定输出文件文件名，则outresultf默认为fhead_simout.txt；
	if (LONG_INDEL){
		indelfp = FileOpen( indelf,  "r");//以只读方式打开插入缺失设置文件；
		sprintf(outindelf, "%s.idx", outsimf);
		outindelfp = FileOpen( outindelf,  "w"); //以只写方式打开输出流到文件outindelf，并将结果（是否成功）赋给outindelfp；
	} else indelfp = NULL;
	if (OUT_AB) {
...
//#define CLEAR_MUTSEQ(mseq) free( (mseq).s )
	...
	REF_LEN = seq_read_fasta(ingenfp, &seq, ref_name, 0);//读取.fa文件;//ingenfp：读取源；//&seq： //ref_name：//0 //将.fa文件中信息读入seq;
	printf("seq_read_fasta() return! REF_LEN = %d\n\n", REF_LEN);

	//* init sim and set long DEL */ 
	init_indel_sim(&seq, &sim[0], &sim[1], &indelp);	//将seq给入进行indel处理；
	...
	print_simfile( &sim[0], &sim[1], ref_name, &indelp);//输出；
